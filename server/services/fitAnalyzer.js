// Fit calculation logic (server-side version)
export const calculateFitScore = (userMeasurements, product, userPreferences = {}) => {
  if (!userMeasurements || !product) return { overallScore: 0 }

  const { measurements: userMeas, preferences = {} } = userMeasurements
  const productMeas = product.measurements

  if (!userMeas || !productMeas) return { overallScore: 0 }
  ...
  const fitPreference = preferences[product.category] || userPreferences[product.category] || 'regular'
...
const getBrandFitAdjustment = (brand, category) => {
  const brandAdjustments = {
    'Chanel': { tops: 5, dresses: 5, bottoms: 0, outerwear: 3 },
    'Hermes': { tops: 3, dresses: 3, bottoms: 2, outerwear: 2 },
    'Prada': { tops: -2, dresses: -2, bottoms: -3, outerwear: -2 },
    'Gucci': { tops: 0, dresses: 0, bottoms: -2, outerwear: 0 },
    'Saint Laurent': { tops: -5, dresses: -5, bottoms: -5, outerwear: -3 }
  }
